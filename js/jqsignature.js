...
var WritingPad = function () {
    ...
    $(function () {
        ...
        $(document).on("click", "#mySave", null, function () {
            ...
            var img = $('<img class="Mysignature">').attr('src', dataUrl);
            var Signature=$('<img>').attr('src','img/sp.png');
            var spname=$('<p class="spname">').text("审批人:");
            var spriqi=$('<p class="spriqi">').text("审批日期:");
            $(".Mysignature").appendTo($(".spname"));
            $(myImg).append(spname);
            $(myImg).append(spriqi);
            $(spname).append(img);
            $(myImg).append(Signature);
//          $(myImg).append(img);
    ...
    function initTable() {
        ...
        + colorTable + '</table>' +
        '<table width=241 border="0" cellspacing="0" cellpadding="0" style="border:1px #000000 solid;border-collapse: collapse;background-color:#000000">' +
        '<tr style="height:30px">' +
        '<td colspan=21 bgcolor=#cccccc>' +

        '<table cellpadding="0" cellspacing="1" border="0" style="border-collapse: collapse">' +
        '<tr>' +
        '<td width="3"><input type="text" name="DisColor" size="6" disabled style="border:solid 1px #000000;background-color:#ffff00" style="margin-left:10px"></td>' +
        '<td width="3"><input type="text" name="HexColor" size="7" style="border:inset 1px;font-family:Arial;margin-left:10px;" value="#000000"></td>' +
         '<td width="3"><button type="button" class="btn btn-primary btn-sm" id="btnSave" style="width:60px;height:25px;margin-left:10px">确认</button></td>' +
        '</tr>' +
        '</table>' +
...
//初始化面板
    function initSignature() {
        ...
        if (window.requestAnimFrame) {
            ...
            signature.jqSignature({ width:300, height:150, border: '1px solid red', background: '#FFFFF', lineColor: '#ABCDEF', lineWidth: 2, autoFit: false });
